...
//import quickfix.mina.CriticalProtocolCodecException;  //TEST
import org.apache.mina.filter.codec.ProtocolCodecException;
...
public class FIXMessageDecoder implements MessageDecoder {
    ...
    private void handleError(ByteBuffer buffer, int recoveryPosition, String text,
        ...
        bodyLength = 0;
        if (disconnect) {
            //throw new CriticalProtocolCodecException(text);  //TEST
            throw new ProtocolCodecException(text);
        } else {
            log.error(text);
        }
    }
    ...
    private boolean isLogon(ByteBuffer buffer) {
        //System.out.println("Decoder+isLogon");
